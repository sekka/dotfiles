...
| `/user-pmo-excel`     | Excel → YAML 一方向 pull     | WBS.xlsx を真実とし、`pull` で `WBS.yaml` の `tasks` ブロックを再生成 (AI 可読の派生物)。`init` / `doctor` も提供。ガントチャート部分は対象外 |
...
/user-pmo-excel    pull: ~/prj/{name}/WBS.xlsx → WBS.yaml (一方向。Excel が真実、YAML は派生物)
```
