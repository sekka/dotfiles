...
| `/user-pmo-excel`     | Excel ⇄ YAML 一方向同期      | WBS.xlsm と `pmo.yaml` を snapshot ベースで同期。方向は明示: `pull` (Excel→YAML) / `push` (YAML→Excel)。ガントチャート部分は VBA に委ねる |
...
/user-pmo-excel    pull: ~/prj/{name}/WBS.xlsm → pmo.yaml  /  push: pmo.yaml → WBS.xlsm
```
