...
| `/user-doc-design-spec`                   | テキスト設計         | UI設計ドキュメント生成         | テキストワイヤーフレーム・レイアウト仕様・デザインシステム定義（色/タイポ/トークン）・コンポーネント状態一覧などをMarkdownで作成。コード生成は行わない（コード生成は `frontend-design` スキルを使用） |
