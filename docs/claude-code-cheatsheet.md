...
データストア: `~/prj/{name}/WBS.yaml`（案件ごと）/ `~/prj/members.yaml`（メンバー定義・共通）
...
| `/user-pmo-status`    | 複数案件進捗ダッシュボード   | **週次確認時**。`~/prj/*/WBS.yaml`を読んで全案件の進捗・期限・アラートを一覧表示。2週以内の期限は🔴                                       |
| `/user-pmo-workload`  | メンバー稼働可視化           | アサイン調整時。メンバー別の週次割当時間・負荷率（%）を表示し、過負荷メンバーの調整案を提示                                               |
| `/user-pmo-excel`     | Excel ⇄ YAML 一方向同期      | WBS.xlsm と `WBS.yaml` を snapshot ベースで同期。方向は明示: `pull` (Excel→YAML) / `push` (YAML→Excel)。ガントチャート部分は VBA に委ねる |

**PMO データフロー:**

```
/user-pmo-wbs      → ~/prj/{name}/WBS.yaml を作成・更新
/user-pmo-status   ← ~/prj/*/WBS.yaml を読み取り
/user-pmo-workload ← ~/prj/*/WBS.yaml + ~/prj/members.yaml を読み取り
/user-pmo-excel    pull: ~/prj/{name}/WBS.xlsm → WBS.yaml  /  push: WBS.yaml → WBS.xlsm
```

### PMセッション・案件管理（user-pm-*）

データストア: `~/prj/{slug}/discovery.md` / `spec.md` / `design-doc.md` / `decisions.md` / `WBS.yaml`

| スキル              | 説明                        | いつ使う？                                                                                                      |
| ------------------- | --------------------------- | --------------------------------------------------------------------------------------------------------------- |
| `/user-pm-session`  | PMセッション開始・診断      | **PM作業開始時**。全プロジェクトをスキャンし、期限・リスク・未解決事項を自動診断してアクション優先順位を提示    |
| `/user-pm-discover` | 要件ヒアリング（Discovery） | **新案件開始時**。7質問インタビューを実施し、`discovery.md`と初期`WBS.yaml`を生成。ゴール・制約・リスクを収集   |
| `/user-pm-spec`     | 仕様書・設計書生成          | `discovery.md`完成後。RTM（要件トレーサビリティ）`spec.md`とデザインドキュメント`design-doc.md`を生成           |
| `/user-pm-meeting`  | 議事録の構造化              | 会議後にメモを渡すと決定事項・アクション・未解決・リスクを抽出し`decisions.md`と`WBS.yaml`を更新                |
| `/user-pm-report`   | 週次ステータスレポート生成  | 週次報告・マイルストーン確認時。`WBS.yaml`と`decisions.md`を集計しCSV＋Notion出力（notion_page_id設定時）       |
| `/user-pm-judge`    | PMとして何をすべきか判断    | 役割が曖昧なとき・「嫌な予感がする」とき。Player/PM/Mixedモードを判定し10のPM鉄則から15分以内のアクションを提示 |

**PM データフロー:**

```text
/user-pm-discover  → ~/prj/{slug}/discovery.md + WBS.yaml を作成
/user-pm-spec      → ~/prj/{slug}/spec.md + design-doc.md + WBS.yaml を作成
/user-pm-meeting   → ~/prj/{slug}/decisions.md + meetings/ + WBS.yaml を更新
/user-pm-report    ← ~/prj/{slug}/WBS.yaml + decisions.md を読み取り → report-YYYY-MM-DD.csv
/user-pm-session   ← ~/prj/*/WBS.yaml + decisions.md を読み取り
```
...
| 議事録を構造化             | `/user-pm-meeting`  | メモ → 決定事項・アクション・WBS.yaml更新 |
| 週次レポートを生成         | `/user-pm-report`   | WBS.yaml集計 → CSV + Notion出力           |
