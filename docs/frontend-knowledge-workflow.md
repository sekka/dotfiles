...
`user-doc-fe-kb` スキルと qmd を使った日常運用の手順書。
...
1. `WebFetch` で取得
2. `~/.claude/skills/user-doc-fe-kb/raw/_inbox/YYYY-MM-DD-{slug}.md` に原文保存
...
```bash
cd ~/dotfiles
git add home/.claude/skills/user-doc-fe-kb/raw/_inbox/
...
```bash
cd ~/dotfiles
git add home/.claude/skills/user-doc-fe-kb/
...
- `home/.claude/skills/user-doc-fe-kb/SKILL.md` — Claude セッション内のモード定義（収集/コンパイル/整理）
- `home/.claude/skills/user-doc-fe-kb/raw/README.md` — raw/ 層の構造・命名規則
