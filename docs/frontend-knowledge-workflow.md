...
`user-fe-knowledge` スキルと qmd を使った日常運用の手順書。
...
1. `WebFetch` で取得
2. `~/.claude/skills/user-fe-knowledge/raw/_inbox/YYYY-MM-DD-{slug}.md` に原文保存
...
```bash
cd ~/dotfiles
git add home/.claude/skills/user-fe-knowledge/raw/_inbox/
...
```bash
cd ~/dotfiles
git add home/.claude/skills/user-fe-knowledge/
...
| 完全リセット             | `qmd collection remove frontend && ~/dotfiles/setup/21_qmd.sh`    |
| インデックス破損疑い     | `qmd cleanup && ~/dotfiles/setup/21_qmd.sh`                       |
| `_inbox/` 肥大化         | 処理停滞サイン。「inbox 処理して」を Claude に依頼                |
| 埋め込み生成途中で失敗   | `~/dotfiles/setup/21_qmd.sh` を再実行（差分のみ再生成、復旧可能） |

---

## 関連ドキュメント

- `setup/setup-qmd.md` — qmd セットアップの詳細・コマンドリファレンス（`setup/21_qmd.sh` 用）
- `home/.claude/skills/user-fe-knowledge/SKILL.md` — Claude セッション内のモード定義（収集/コンパイル/整理）
- `home/.claude/skills/user-fe-knowledge/raw/README.md` — raw/ 層の構造・命名規則
