...
| `Ctrl+T` | ファイル選択     | fzf でファイルを選択してコマンドラインに挿入 |
| `Alt+C`  | ディレクトリ移動 | fzf でディレクトリを選択して移動             |
...
| `y [path]`              | yazi を起動（終了時にディレクトリ移動） | `y`                     |
| `wk [path]`             | walk でディレクトリを選択して移動       | `wk`                    |
...
| エイリアス | コマンド            | 説明                                    |
| ---------- | ------------------- | --------------------------------------- |
| `y`        | yazi wrapper        | yazi を起動（終了時にディレクトリ移動） |
| `yz`       | `y`                 | y のショートカット                      |
| `yy`       | `yazi`              | ディレクトリ移動なし                    |
| `yn`       | `yazi --no-preview` | プレビューなし（軽量動作）              |
| `sf`       | `spf`               | TUI ファイルマネージャ（superfile）     |
...
| エイリアス | コマンド               | 説明                       |
| ---------- | ---------------------- | -------------------------- |
| `qmd-fe`   | `INDEX_PATH="..." qmd` | フロントエンドナレッジ検索 |
...
`cd` は zoxide に統合済み。引数なしで fzf インタラクティブ選択、`cdi` でも選択可能。
...
| エイリアス | コマンド             | 説明                                   |
| ---------- | -------------------- | -------------------------------------- |
| `dn`       | `diffnav`            | delta 連携 diff ナビゲータ             |
| `sc`       | `silicon --font ...` | コードスニペットを画像としてキャプチャ |
...
| コマンド       | 説明                         | 提供元    |
| -------------- | ---------------------------- | --------- |
| `cd <keyword>` | ディレクトリジャンプ         | zoxide    |
| `cdi`          | fzf でインタラクティブに選択 | zoxide    |
...
| `Prefix w`     | ウィンドウ選択   | tcmux でエージェントステータス付きウィンドウ選択 |
...
| キー          | 機能       | 説明                                                  |
| ------------- | ---------- | ----------------------------------------------------- |
| `Shift+Enter` | 複数行入力 | Kitty keyboard protocol リレー（AI エージェント対応） |
...
| `Prefix Ctrl+K` | keifu popup   | keifu を popup 表示              |
...
tmux 用サイドバープラグイン。セッション切り替え・エージェント状態・リポジトリ情報を26列の固定ペインで表示。Claude Code / Codex / Amp / OpenCode のエージェントウォッチャーを内蔵。

| キー             | 機能                       | 説明                                       |
| ---------------- | -------------------------- | ------------------------------------------ |
| `Prefix o → s`   | サイドバー表示・フォーカス | サイドバーを開いてフォーカスを移動         |
| `Prefix o → t`   | サイドバー トグル          | サイドバーの表示/非表示を切り替え          |
| `Prefix o → 1-9` | セッション番号ジャンプ     | 表示中のセッションをインデックスで切り替え |
