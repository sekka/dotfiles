...
if (!commitMsgFile) {
	console.error("ERROR [Git Hook] コミットメッセージファイルが指定されていません");
	process.exit(1);
}
...
try {
	...
	if (!result.valid) {
		console.error("\x1b[0;31mERROR [Git Hook] コミット検証エラー:\x1b[0m");
	...
	console.error(`\x1b[0;31mERROR [Git Hook] ファイル読み込みエラー: ${commitMsgFile}\x1b[0m`);
