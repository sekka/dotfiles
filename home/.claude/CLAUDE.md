...
詳細: `@.claude/rules/tdd-workflow.md`
...
実装タスク開始
  ↓
要件は明確？ NO → AskUserQuestion
  ↓ YES
設計判断が必要？ YES → /ask-peer
  ↓ NO
実装実行（随時 /reviewing-with-claude）
  ↓
実装完了
  ↓
重要な変更？ YES → /reviewing-parallel
           NO  → /reviewing-with-claude
```
...
### AskUserQuestion の効果的な使い方

**使用すべき場面**:
1. 曖昧な要件（"パフォーマンスを改善して" → 何を優先？）
2. 優先順位の確認（トレードオフがある場合）
3. 制約条件の把握（納期、スキル、互換性）
4. 最終判断の委譲（AI が勝手に決めない）

**効果的な質問の条件**:
- 選択肢は 2-4 個
- 自明でない質問
- 具体的で判断可能
- 必要に応じて `multiSelect: true` を使用
...
**トリガーキーワード**:
- **CSS**: Grid、Flexbox、アニメーション、@scope、Popover、light-dark
- **JavaScript**: DOM操作、async/await、IntersectionObserver、デザインパターン
- **HTML**: セマンティック、dialog、details、フォーム
- **横断的**: パフォーマンス、アクセシビリティ、ブラウザ互換性
- **デザイン**: Apple Style Guide、HIG、Material Design 3

**参照方法**:
1. 質問からキーワードを抽出
2. カテゴリを特定（css/layout、javascript/patterns など）
3. ナレッジファイルを Read（`~/.claude/skills/managing-frontend-knowledge/knowledge/[カテゴリ]/[ファイル].md`）
4. ナレッジに基づいて回答（出典 URL、ブラウザサポート情報を明記）
