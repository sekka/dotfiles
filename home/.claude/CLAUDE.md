...
- **全体を見る**：言われた箇所だけでなく、変更が他のページ・他のコンポーネントに波及しないか必ず考慮する。**一箇所直して別の箇所を壊す場当たり修正は絶対にしない。** 不確かなら修正前に全体設計を理解してから1回で正しく行う
...
- ブラウザ自動化はURLを受け取ったら躊躇わず使用する。提案ではなく実行する（詳細: `.claude/rules/browser-automation.md`）
