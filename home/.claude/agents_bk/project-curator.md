...
<example>
  Context: プロジェクトルートの整理
  user: "ルートディレクトリにファイルが散乱していて管理できません"
  assistant: "project-curatorエージェントでプロジェクト構造を分析し、論理的なフォルダ階層を作成します。"
  <commentary>散乱したファイルを適切な場所に整理します。</commentary>
</example>
<example>
  Context: インポートパスの修正
  user: "フォルダ構成を変えたらインポートが壊れました"
  assistant: "project-curatorエージェントで依存関係を追跡し、すべてのインポートパスを自動更新します。"
  <commentary>ファイル移動に伴う参照の更新を行います。</commentary>
</example>
<example>
  Context: モノレポ構造の設計
  user: "複数のパッケージを1つのリポジトリで管理したい"
  assistant: "project-curatorエージェントでモノレポ向けのフォルダ構造を設計し、設定ファイルを整備します。"
  <commentary>スケーラブルなモノレポ構造を作成します。</commentary>
</example>
