...
エラー・不明点が発生
  ↓
ローカルコードベース内の検索？
  YES → serena（シンボル操作）/ grepai（セマンティック）/ Grep（正確）/ Glob（ファイル名）
  NO  → 次へ
  ↓
公式ドキュメントが必要？
...
- シンボル単位での検索・操作（クラス、関数、メソッド）
- リファクタリング（シンボルの置き換え、リネーム）
- 構造的な参照元検索

**使用例:**

```
質問: "UserService クラスの構造を知りたい"
→ serena get_symbols_overview

質問: "registerUser 関数を呼んでいるコードは？"
...
- シンボル名が分かっている場合に最適
- 編集機能も提供（置き換え、挿入、リネーム）

**詳細:** Serena Instructions Manual（システムプロンプト内）

#### grepai（セマンティック検索）

**用途:**

- 自然言語での概念検索（"認証ロジック"、"エラーハンドリング"）
- コールグラフ（関数の呼び出し元・呼び出し先の追跡）
- 類似コードの発見
...
```
質問: "ユーザー認証のコードを探して"
→ grepai search "user authentication code"

質問: "saveUser 関数を呼んでいるコードは？"
...
- Ollama が起動している必要がある（`ollama serve`）
- プロジェクトで事前に `grepai init && grepai watch` が必要
- 100% ローカル実行（プライバシー保護）

**詳細:** `@.claude/rules/semantic-search.md`
...
| 検索内容 | 最適ツール | 理由 |
|----------|------------|------|
| "UserService クラスの構造" | **serena** | シンボル単位で構造把握 |
| "認証ロジック" | **grepai** | 自然言語での概念検索 |
| `"function handleError"` | **Grep** | 正確なテキスト |
| `"*.ts"` ファイル一覧 | **Glob** | ファイル名検索 |
| "この関数の呼び出し元" | **serena** または **grepai** | 両方試して比較 |
| "リファクタリング" | **serena** | 編集機能あり |

**詳細:** `@.claude/rules/semantic-search.md`（serena と grepai の詳細な使い分け）
