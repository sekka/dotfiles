...
ドキュメント内に登場する専門用語・業界用語を整理し、付録として追加する。

#### 用語集作成の原則

1. **一般用語と独自用語の分離**
   - 一般的な業界用語: まとめて1つのテーブル
   - クライアント独自用語・プロダクト名: 別セクションで明示

2. **対象となる用語**
   - 業界特有の専門用語（例: リースアップ、求償、陸送）
   - ドキュメント内で繰り返し登場する重要概念
   - 一般読者には馴染みがない可能性のある用語
   - 理解を深めるために知っておくべき関連用語

3. **対象外となる用語**
   - クライアント独自のプロダクト名・サービス名（別セクション化）
   - 一般的すぎる用語（「会社」「顧客」など）
   - ドキュメント内で1回のみ登場する用語
...
- 簡潔（1-2文で完結）
- 具体的（抽象的な説明を避ける）
- 文脈に即している（ドキュメントの文脈で理解できる）
- 必要に応じて補足情報を追加（例: 市場規模、関連概念）

**例**:

```markdown
| **リースアップ** | リース期間終了により、車両がリース会社に返却されること |
| **求償** | 損傷に対する費用請求 |
| **オートオークション** | 業者間で車両を売買する市場。全国に120〜130会場、年間出品台数800万台 |
```
...
1. **最優先**: ドキュメントの核心概念（例: プロダクト名、主要機能）
2. **優先**: 3回以上登場する業界用語
3. **必要に応じて**: 理解を助ける関連用語（ドキュメント内で直接言及されていなくても）

#### 用語の配置順序

**推奨順序**:
1. プロセス順（業務フローに沿って）
2. カテゴリ別（リース関係、オークション関係など）
3. あいうえお順（辞書的）
...
| 用語 | 説明 |
|------|------|
${generalTerms.map(term => `| **${term.name}** | ${term.description} |`).join('\n')}
...
| 用語 | 説明 |
|------|------|
${proprietaryTerms.map(term => `| **${term.name}** | ${term.description} |`).join('\n')}
` : ''}
`;

// ドキュメント末尾に追加
appendToDocument(glossary);
```
