...
After the user confirms the summary, automatically check the following and flag any issues before generating files:

- **Goals ↔ Requirements consistency**: every Goal must have at least one corresponding Requirement; flag Goals with no supporting Requirement
- **Non-Goals ↔ Constraints overlap**: check for implicit constraints that should have been listed as Non-Goals
- **Risks ↔ Open Questions duplication**: flag items that appear in both (usually one location is sufficient)
- **Approver set**: if approver is empty or not provided, prompt: "承認者が未設定です。確認してください。"

If issues found, output:
```
AI Review: {N} item(s) flagged. Please confirm before files are generated.
- {issue 1}
- {issue 2}
```
...
If no issues:
```
AI Review: No issues found. Proceeding to generate files.
```
