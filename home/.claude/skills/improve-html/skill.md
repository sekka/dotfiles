...
- Bun がインストール済み
- `scripts/development/check-html.ts` が利用可能
...
URL入力の場合、ブラウザツール選択基準は `~/.claude/rules/browser-automation.md` に従う。
...
```bash
bun scripts/development/check-html.ts <file> --format=json --severity=info
```
...
- APGパターン: `patterns/*.md`
- CLIスクリプト: `scripts/development/check-html.ts`
