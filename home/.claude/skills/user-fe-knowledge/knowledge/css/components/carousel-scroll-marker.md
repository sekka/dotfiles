...
tags: [carousel, scroll-snap, pseudo-elements, accessibility, scroll-button, scroll-marker, target-current, 2025, 2026]
...
> 出典:
> - https://ics.media/entry/250516/ (執筆 2025-05-16)
> - https://zenn.dev/chot/articles/bb66d5288edd03 — Yuki Shibata / chot Inc. (公開 2026-04-13)
>
> 追加日: 2025-02-01 / 更新: 2026-05-13
...
### scroll-marker-group プロパティと tabs キーワード

`scroll-marker-group` プロパティで、マーカー要素の配置位置とキーボード操作を制御できる。

```css
.carousel {
  /* マーカーを後ろに配置 + tabs キーワードで Tab キー操作有効化 */
...
**重要ポイント**:
- `:target-current` で現在表示中のマーカーをスタイリング（旧 `:checked` は誤用、`:target-current` が正式）
- `scroll-marker-group: after tabs` で Tab キーによるキーボードナビゲーションを自動有効化
- `::scroll-marker` / `::scroll-button` は `content` プロパティが `none` だと生成されない（`::before` / `::after` と同じ仕様）
