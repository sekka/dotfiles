...
2. WebFetch失敗（403/429/空レスポンス） → pinchtab で取得
3. pinchtab失敗またはログイン要 → chrome-devtools MCP で試行
4. 全て失敗 → 失敗理由をユーザーに報告して終了
```

**Quick評価時**: WebFetch のみ試行（高速優先）、失敗時のみ pinchtab へフォールバック
**Deep調査時**: 全スキーム試行可
