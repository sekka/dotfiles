...
**⚠️ 絶対禁止: Figma を確認せずにコーディングしてはいけない。**
内部知識や推測でデザインを補完せず、必ず Figma MCP で該当ノードを確認してから実装する。

各セクション・コンポーネントを実装する際は以下のループを繰り返す:

```
[セクションごとの実装ループ]
1. mcp__figma__get_design_context でそのセクションのノードを確認
2. mcp__figma__get_screenshot で視覚確認
3. 確認した情報をもとに実装
4. 次のセクションへ → 1 に戻る
```
