...
## まず get_variable_defs を使う

`mcp__figma__get_variable_defs` を呼び出すことで、選択ノードで使用されている色・スペーシング・タイポグラフィ変数を一括取得できる。手動照合が不要になる。

```text
mcp__figma__get_variable_defs → 変数定義を直接取得
```

取得できた場合はそのまま実装に利用する。以下の手動照合は `get_variable_defs` が使えない場合のフォールバック。
