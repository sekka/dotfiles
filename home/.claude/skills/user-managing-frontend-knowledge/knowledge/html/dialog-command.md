...
> 出典: https://ics.media/entry/260209/
> 執筆日: 2025年（推定）
> 追加日: 2026-04-13

`--` プレフィックスでカスタムコマンドを定義し、離れたコンポーネント間でイベントを発行できます。CSS カスタムプロパティと同様の命名規則です。

```html
<button commandfor="sidebar" command="--toggle-sidebar">
  サイドバー切り替え
</button>
...
```javascript
// カスタムコマンドをリッスン
document.getElementById('sidebar').addEventListener('command', (e) => {
...
グローバル状態（Redux、Context API）に依存せず、`command` イベントでコンポーネント間通信を実装できます。
...
function Sidebar() {
  ...
  useEffect(() => {
    ...
    return () => el.removeEventListener('command', handler);
  }, []);

  return <div id="sidebar" ref={ref}>...</div>;
}
...
**メリット**: 状態管理ライブラリ不要・コンポーネントが疎結合に保てる
